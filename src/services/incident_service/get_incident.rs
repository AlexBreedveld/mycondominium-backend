...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Got Incidents successfully", body = IncidentListHttpResponse, headers(
...
pub async fn get_incidents(
    ...
    let total_items = match incident_model::IncidentModel::db_count_all_matching(role.clone(), conn)
    {
        Ok(res) => res,
        Err(e) => {
            ...
            return HttpResponse::InternalServerError().json(HttpResponseObjectEmptyError {
                error: true,
                message: "Error getting Incidents".to_string(),
            });
        }
    };

    match incident_model::IncidentModel::db_read_all_matching_by_range(role, conn, per_page, offset)
    {
        Ok(res) => {
            ...
            HttpResponse::Ok()
                ...
                .json(HttpResponseObject {
                    error: false,
                    message: "Got Incidents successfully".to_string(),
                    object: Some(res),
                })
        }
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(HttpResponseObjectEmpty {
                error: true,
                message: format!("Error getting Incidents: {}", e),
            })
        }
    }
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Got Incident successfully", body = IncidentGetHttpResponse),
        (status = 400, description = "Invalid Incident ID format or Incident ID is required", body = HttpResponseObjectEmptyError),
        (status = 401, description = "Unauthorized", body = HttpResponseObjectEmptyError),
        (status = 500, description = "Internal server error", body = HttpResponseObjectEmptyError)
    ),
    security(
...
pub async fn get_incident_by_id(
    ...
    match incident_model::IncidentModel::db_read_by_id_matching_resident(role, conn, id) {
        Ok(user_req) => HttpResponse::Ok().json(HttpResponseObject {
            error: false,
            message: "Got Incident successfully".to_string(),
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(HttpResponseObjectEmpty {
                error: true,
                message: format!("Error getting Incident: {}", e),
            })
        }
    }
}
