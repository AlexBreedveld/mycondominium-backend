...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Got Reservations successfully", body = ReservationListHttpResponse, headers(
...
pub async fn get_reservations(
    ...
    let total_items =
        match reservation_model::ReservationModel::db_count_all_matching(role.clone(), conn) {
            Ok(res) => res,
            Err(e) => {
                log::error!("Error getting Reservations: {}", e);
                return HttpResponse::InternalServerError().json(HttpResponseObjectEmptyError {
                    error: true,
                    message: "Error getting Reservations".to_string(),
                });
            }
        };

    match reservation_model::ReservationModel::db_read_all_matching_by_range(
        ...
        Ok(res) => {
            ...
            HttpResponse::Ok()
                ...
                .json(HttpResponseObject {
                    error: false,
                    message: "Got Reservations successfully".to_string(),
                    object: Some(res),
                })
        }
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(HttpResponseObjectEmpty {
                error: true,
                message: format!("Error getting Reservations: {}", e),
            })
        }
    }
}
...
#[utoipa::path(
    ...
    responses(
        (status = 200, description = "Got Reservation successfully", body = ReservationGetHttpResponse),
        (status = 400, description = "Invalid Reservation ID format or Reservation ID is required", body = HttpResponseObjectEmptyError),
        (status = 401, description = "Unauthorized", body = HttpResponseObjectEmptyError),
        (status = 500, description = "Internal server error", body = HttpResponseObjectEmptyError)
    ),
    security(
...
pub async fn get_reservation_by_id(
    ...
    let id = match Uuid::parse_str(&id) {
        ...
        Err(_) => {
            return HttpResponse::BadRequest().json(HttpResponseObjectEmpty {
                error: true,
                message: "Invalid Reservation ID format".to_string(),
            });
        }
    };
    ...
    match reservation_model::ReservationModel::db_read_by_id_matching(role, conn, id) {
        Ok(user_req) => HttpResponse::Ok().json(HttpResponseObject {
            error: false,
            message: "Got Reservation successfully".to_string(),
        ...
        Err(e) => {
            ...
            HttpResponse::InternalServerError().json(HttpResponseObjectEmpty {
                error: true,
                message: format!("Error getting Reservation: {}", e),
            })
        }
    }
}
